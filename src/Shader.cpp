...
Shader::Shader(std::string_view vertex_shader, std::string_view fragment_shader)
  ...
  std::copy(std::istreambuf_iterator<char>(vert_shader_file), {},
            std::back_inserter(vertex_code));
  std::copy(std::istreambuf_iterator<char>(frag_shader_file), {},
            std::back_inserter(fragment_code));
...
unsigned int Shader::compileShader(const std::string& shaderCode,
                                   ShaderType type) {
...
void Shader::use() const noexcept { glUseProgram(program_); }
}  // namespace glewext