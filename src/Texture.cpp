...
namespace glewext {
Texture::Texture() : texture_id_(0u), width_(0), height_(0) {
...
void Texture::load_image_from_memory(TextureLevel level,
                                     TextureInternalFormat internal_format,
                                     int width, int height,
                                     TextureBorder border, TextureFormat format,
                                     GLType gl_type, const void* data) {
  width_ = width;
  height_ = height;
  glBindTexture(GL_TEXTURE_2D, texture_id_);

  glTexImage2D(GL_TEXTURE_2D, static_cast<int>(level),
               static_cast<int>(internal_format), width_, height_,
               static_cast<int>(border), static_cast<int>(format),
               static_cast<int>(gl_type), data);
